...
function getUser_subs($google_id) {
  ...
  // Get sub & sub_details
  $stmt = $pdo->prepare("SELECT `sub_details_id`, `expiry` FROM subs WHERE account_id = ?");
  ...
  // Get sub details
  $stmt = $pdo->prepare("SELECT `name` FROM sub_details WHERE id = ?");
  $stmt->execute([ $sub_data['sub_details_id'] ]);

  if($stmt->rowCount() == 0) return null;
  
  $sub_details = $stmt->fetch(PDO::FETCH_ASSOC);

  unset($sub_data['sub_details_id']);
  
  $sub_data['name'] = $sub_details['name'];
